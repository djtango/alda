  ...
  (let [new-instruments (into {}
                          (for [name names
                            ...
                            (let [{:keys [id] :as inst} (new-part name)]
                              [id inst])))
        instruments     (merge instruments new-instruments)
        instances (->> (for [name names]
                         (get nicknames name (if nickname
                                               (look-up new-instruments name)
                                               (look-up instruments name))))
                       flatten
                       (remove nil?))
        nicknames (when nickname
...
(defmethod update-score :part
  ...
  (let [score  (determine-current-instruments score instrument-call)
        events (cons (apply-global-attributes)
                     (interpose (apply-global-attributes) events))]
